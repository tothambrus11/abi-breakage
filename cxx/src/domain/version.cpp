...
bool DebianVersion::is_prerelease() const noexcept { return upstream_.contains('~'); }
...
ReleaseLevel parse_release_level(std::string_view s) noexcept {
...
ReleaseLevel release_level(std::string_view from, std::string_view to) noexcept {
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    const auto vb = i < b.parts.size() ? b.parts[i] : 0;
    if (va != vb) {
      if (i == 0)
