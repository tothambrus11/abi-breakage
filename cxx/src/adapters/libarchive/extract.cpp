...
struct Nested {
  archive *outer;
  std::array<char, std::size_t{1} << 16U> buf;
};
...
Result<ports::ExtractStats> LibarchiveExtractor::extract(
  ...
  if (archive_read_open_filename(outer.get(), deb.c_str(), std::size_t{1} << 16U) != ARCHIVE_OK)
...
Result<std::string> read_maybe_compressed(const std::filesystem::path &p) {
  ...
  if (archive_read_open_filename(a.get(), p.c_str(), std::size_t{1} << 16U) != ARCHIVE_OK)
