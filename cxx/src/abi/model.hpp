...
// vocabulary the whole study is expressed in; every other module either
// produces ChangeKind values (abi/, hdr/) or consumes them (pipeline/analyze).
// =============================================================================
...
enum class ChangeKind : std::uint8_t {
  // -- layout (visible in DWARF, absorbed by opaque layout)
  ...
  type_size_changed,
  base_class_changed,
  // -- enums (visible in DWARF, absorbed by non-frozen enums)
  enum_case_added,
  enum_case_removed,
  // -- dispatch (visible in DWARF, absorbed by resilient dispatch)
  vtable_changed,
  // -- symbols (visible in ELF; source-level API changes, not absorbable)
  function_signature_changed,
  symbol_removed,
  symbol_added,
  symbol_version_renamed,
  // -- header bodies (invisible to every ABI tool; absorbed by opt-in inlining)
  inline_body_changed,
  macro_value_changed,
};
