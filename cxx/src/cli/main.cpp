// =============================================================================
// abistudy command line. Parses arguments, builds the Workspace and client,
// dispatches to a pipeline stage, and turns the stage's Result into an exit
// code. No analysis logic lives here.
// =============================================================================
...
using namespace abistudy::pipeline;
...
struct Args {
  ...
  /// @brief Option value or default. A flag given without a value is "".
...
Result<void> run(const Args &a) {
  ...
  const auto do_select = [&]() -> Result<void> {
    ...
    ABISTUDY_TRY_VOID(run_select(ws, client, o, log));
    return {};
  };
  const auto do_resolve = [&]() -> Result<void> {
    ...
    ABISTUDY_TRY_VOID(run_resolve(ws, client, o, log));
    return {};
  };
  const auto do_diff = [&]() -> Result<void> {
    DiffOptions o;
    ABISTUDY_TRY(o.workers, a.uint("workers", o.workers));
    ABISTUDY_TRY(auto secs, a.uint("pair-timeout", 3600));
    ...
    ABISTUDY_TRY(auto cm, a.uint("child-memory-mb", 5000));
    ...
    if (a.has("one")) {
      ...
      ABISTUDY_TRY_VOID(diff_one(ws, client, plan.jobs[i], o, log));
      return {};
    }
    std::error_code ec;
  ...
  if (a.stage == "analyze")
    return do_analyze();
  if (a.stage == "report") {
    ABISTUDY_TRY_VOID(run_report(ws, log));
    return {};
  }
  if (a.stage == "all") {
