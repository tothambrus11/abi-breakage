...
# A symbol printed as `name@@LIBFOO_PRIVATE_1.15.2` sits in an ELF version node
# the library marked private. dbus exports 657 such `_dbus_*` internals against
# 240 public ones, and a routine release then reads as "651 public functions
# removed". This is a generic, header-free rule: no per-library configuration.
PRIVATE_VERSION_NODE = re.compile(r"@@?[A-Za-z0-9_.]*(?:PRIVATE|INTERNAL)", re.I)


def is_private_symbol(sym):
    return bool(sym) and bool(PRIVATE_VERSION_NODE.search(sym))
